...
class STFLNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new STFLNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawSTFLNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : STFLNode {
        var curr : STFLNode = this.prev 
        if (dir == 1) {
            curr = this.next
        }
        if (curr) {
...
class SquareToFlyLine {
    
    curr : STFLNode = new STFLNode(0)
    dir : number = 1
